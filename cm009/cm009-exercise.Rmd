...
```
For example, parties 3 has at least one member pending for brunch.
...
```
It seems that parties 3, 4, 8 and 10 have a PENDING status for one of their members for at least one event.
...
tibble(mu = mu, sd = sd) %>% 
  group_by_all() %>% 
  mutate(z_scores = list(rnorm(n, mu, sd))) %>% 
  unnest(z_scores)
```
