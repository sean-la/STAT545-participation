...
gapminder %>% 
  ...
  scale_size_area()
```
COMMENT: lets try playing around with the shape and alpha parameters in `geom_point`.
...
gapminder %>% 
  ...
  scale_size_area()
```
This gives an error that pch value `30` is not implemented. The variable must be categorical, not some quantitative.
...
```
The problem is that `size` is interpretted as some category.
Let's try to put `size` outside of `aes`.
